  ...
  describe "Link Headers" do
    ...
    context "when on page 1 of 2" do
      ...
      it "returns a Link header" do
        ...
        headers["Link"].should ==
        '<http://www.example.com/api/search?keyword=parent&page=2&per_page=1>; '+
        'rel="last", '+
        '<http://www.example.com/api/search?keyword=parent&page=2&per_page=1>; '+
        'rel="next"'
      end
    ...
    context "when on page 2 of 2" do
      it "returns a Link header" do
        ...
        headers["Link"].should ==
        '<http://www.example.com/api/search?keyword=parent&page=1&per_page=1>; '+
        'rel="first", '+
        '<http://www.example.com/api/search?keyword=parent&page=1&per_page=1>; '+
        'rel="prev"'
    ...
    context "when on page 2 of 3" do
      it "returns a Link header" do
        ...
        '<http://www.example.com/api/search?keyword=parent&page=1&per_page=1>; '+
        'rel="prev", '+
        '<http://www.example.com/api/search?keyword=parent&page=3&per_page=1>; '+
        'rel="last", '+
        '<http://www.example.com/api/search?keyword=parent&page=3&per_page=1>; '+
        'rel="next"'
        headers["X-Current-Page"].should == "2"
