  ...
  context 'when location has services' do
    ...
    it 'serializes all service attributes and associations' do
      expect(json.first.keys).to eq(
        %w(id accepted_payments alternate_name audience description eligibility
           email fees funding_sources how_to_apply keywords languages name
           required_documents service_areas status website wait_time updated_at
           urls wait categories contacts phones regular_schedules
           holiday_schedules)
      )
    end
  end
