...
describe Ohana::API do
  ...
  describe 'PUT Requests for Services' do
    ...
    describe 'PUT /api/services/:id/' do
      ...
      it 'only replaces service_areas if an array' do
        @service.update!(service_areas: ['Belmont'])
        put(
          "api/services/#{@service.id}/",
          { service_areas: 'Atherton' },
          'HTTP_X_API_TOKEN' => @token
        )
        @service.reload
      ...
      it 'replaces service_areas if an array' do
        @service.update!(service_areas: ['Belmont'])
        put(
          "api/services/#{@service.id}/",
          { service_areas: %w(Atherton Belmont) },
          'HTTP_X_API_TOKEN' => @token
        )
        @service.reload
        expect(@service.service_areas).to eq(%w(Atherton Belmont))
      end

      it 'deletes service_areas if empty array' do
        @service.update!(service_areas: ['Belmont'])
        put(
          "api/services/#{@service.id}/",
          { service_areas: [] },
        ...
        @service.reload
        expect(@service.service_areas).to eq([])
      end
    end
