class WeekdayValidator < ActiveModel::EachValidator
  ...
  WEEKDAY_STRING_NUMS = %w(0 1 2 3 4 5 6).freeze
  ...
  def valid_weekday_num?(value)
    (value.is_a?(Integer) && value.to_i.between?(0, 6)) || valid_string_num?(value)
  end
  ...
  def convert_value_to_integer(record, attribute, value)
    ...
    record[attribute] = DateTime.parse(value).wday if valid_weekday_name?(value)
  end
end
